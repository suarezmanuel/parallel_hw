...
#define N 10000     // node count
#define M 20     // edge count
#define D 0.15   // damping factor
#define I 100    // iterations count
#define BI 10000 // benchmark iterations
#define TC 4     // thread count
#define BLOCK_SIZE (64 / sizeof(float)) // should be 16
...
void initializeRanksStruct(e* ranks) {
    for (int i = 0; i < N; i++) {
        ranks[i/BLOCK_SIZE].data[i%BLOCK_SIZE] = 1.0;
        // ranks[i/BLOCK_SIZE].data[i%BLOCK_SIZE] = 1.0 / N;
    }
}
...
void benchmark2(Graph* graph, float (*f)(Graph*), char* name) {
    double start, end, total = 0;
    ...
    printf("time to calc %-12s  \e[1m%lf\e[m\n", name, total);   
}
...
float t2 (Graph* graph) {
    ...
    for (int iter = 0; iter < I; iter++) 
        // calculate the sum of ranks of those without outlinks
        for (int i=0; i < ceil; i++) {
            for (int j=0; i*BLOCK_SIZE+j < N && j < BLOCK_SIZE; j++) {
                // dont divide outside for precision
                sumB += (graph->adjacencyListsOutLength[i*BLOCK_SIZE+j] == 0) ? ranks[i].data[j]/N : 0;
            }
        }
...
void* worker_t3 (void* args) {
    ...
    // i is a block
        for (int i=wargs->start; i < wargs->end; i++) {
            ...
            for (int j=0; i*BLOCK_SIZE+j < N && j < BLOCK_SIZE; j++) {
                // dont divide outside for precision
                wargs->sum += (wargs->graph->adjacencyListsOutLength[i*BLOCK_SIZE+j] == 0) ? wargs->ranks[i].data[j]/N : 0;
            }
        }

    // each sum to a separate place
    return NULL;
}

float t3 (Graph* graph) {
    // ceil(N/BLOCK_SIZE) structs
    int ceil = (N+BLOCK_SIZE-1) / BLOCK_SIZE;
    e* ranks = calloc(ceil, sizeof(e));
    ...
    worker_args_t args [TC];

    initializeRanksStruct(ranks);
    // printRanksStruct(ranks);
    int step = (ceil+TC-1) / TC;
    int start = 0, end = 0;
    for (int i=0; i < TC; i++) {
        end += step;
        args[i].graph=graph; args[i].ranks=ranks; args[i].start=start; args[i].end=end; args[i].mutex=&mutex; args[i].sum=0.0; args[i].id = i;
        if (pthread_create(&threads[i], NULL, worker_t3, &args[i]) != 0) {
...
int main()
{
    ...
    printf("\n");
    benchmark2(graph, t1, "sum-serial");
    benchmark2(graph, t2, "sum-good");
    benchmark2(graph, t3, "sum-parallel");
    printf("\n");
    // compare2(graph, t1, t2, "sum-serial", "sum-good");
    compare2(graph, t2, t3, "sum-good", "sum-parallel");
    printf("\n");
